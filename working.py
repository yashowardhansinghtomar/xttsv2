...
async def generate_cloned_speech_endpoint(request: GenerateClonedSpeechRequest):
    ...
    # Reuse old voice_id or create a new one
    if request.voice_id and request.voice_id in voice_registry:
        voice_id = request.voice_id
    else:
        voice_id = str(uuid.uuid4())  # Generate new voice ID if not provided
        logging.info(f"New voice_id generated: {voice_id}")
    ...
    try:
        ...
        output_path = f"temp_cloned_{voice_id}_{unique_hash}.{request.output_format}"
...
def generate_tts(text, speaker_wav, language, speed):
    """Handles calling the TTS model properly, whether DataParallel is used or not."""
    with tts_lock:  # Ensure thread-safe access to the TTS model
        model = tts_model.module if isinstance(tts_model, torch.nn.DataParallel) else tts_model
        return model.tts(text=text, speaker_wav=speaker_wav, language=language, speed=speed)
...
async def generate_cloned_speech_endpoint(request: GenerateClonedSpeechRequest):
    ...
    try:
        ...
        # Use ThreadPoolExecutor for parallel processing
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(generate_tts, chunk, speaker_wav, request.language, request.speed) for chunk in text_chunks]
            ...
            for future in as_completed(futures):
                ...
                chunk_audio = normalize_audio(chunk_audio, target_dbfs=-20.0)
        ...
        # Export the final audio with consistent parameters
        final_audio.export(output_path, format=request.output_format.lower(), parameters=["-q:a", "0"])

        with open(output_path, "rb") as audio_file:
            return Response(audio_file.read(), media_type=f"audio/{request.output_format.lower()}")
    finally:
